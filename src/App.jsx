...
// ── InlineCsvRunner — shows inline in response pane ────────────────────────
function InlineCsvRunner({request,envVars,collectionVars,onClose}){
  const[csvRows,setCsvRows]=useState([])
  const[csvHeaders,setCsvHeaders]=useState([])
  const[csvFile,setCsvFile]=useState(null)
  const[running,setRunning]=useState(false)
  ...
  const runAll=async()=>{
    ...
    for(let i=0;i<rows.length;i++){
      ...
      try{
        ...
        const en=(request.params||[]).filter(p=>p.enabled&&p.key)
        if(en.length){const qs=en.map(p=>`${encodeURIComponent(p.key)}=${encodeURIComponent(resolve(p.value,rv))}`).join('&');url=url+(url.includes('?')?'&':'?')+qs}
        const hdrs={}
        ...
        if(auth.type==='apikey'&&auth.key&&auth.in==='header')hdrs[auth.key]=auth.value
        let body=undefined
        if(!['GET','HEAD'].includes(request.method)&&request.bodyType!=='none'&&request.body){hdrs['Content-Type']='application/json';body=resolve(request.body,rv)}
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:'#fff'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'8px 16px',borderBottom:`1px solid ${C.border}`,background:'#f8f8fc',flexShrink:0}}>
        ...
        {csvHeaders.length>0&&apiVars.map(v=>(
          <span key={v} style={{fontSize:10,fontFamily:C.mono,padding:'2px 6px',borderRadius:4,
            background:csvHeaders.includes(v)?'rgba(22,163,74,0.08)':'rgba(220,38,38,0.08)',
            border:`1px solid ${csvHeaders.includes(v)?'rgba(22,163,74,0.25)':'rgba(220,38,38,0.25)'}`,
            color:csvHeaders.includes(v)?C.green:C.red}}>
            {'{{'+v+'}}'} {csvHeaders.includes(v)?'✓':'✗'}
          </span>
        ))}
        <div style={{display:'flex',alignItems:'center',gap:6,marginLeft:'auto'}}>
          <label style={{fontSize:11,color:'#94a3b8'}}>Delay(ms)</label>
          <input type="number" min={0} max={5000} value={delay} onChange={e=>setDelay(Number(e.target.value))} style={{width:60,background:'#fff',border:`1px solid ${C.border}`,borderRadius:5,padding:'2px 6px',fontSize:11,fontFamily:C.mono,outline:'none'}}/>
          {!running
            ?<button onClick={runAll} disabled={!csvFile&&apiVars.length>0} style={{padding:'5px 16px',borderRadius:7,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',border:'none',background:C.green,color:'#fff'}}>▶ Run {csvRows.length>0?csvRows.length+' rows':''}</button>
        ...
        {results.length>0&&(
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead style={{position:'sticky',top:0,background:'#f8f8fc',zIndex:1}}>
              <tr>{['#','Vars','Status','Result','Time'].map(h=><th key={h} style={{padding:'7px 14px',textAlign:'left',fontSize:11,color:'#94a3b8',fontWeight:600,borderBottom:`1px solid ${C.border}`}}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {results.map((r,i)=>{
                const sc=SC(r.status)
...
// ── BackendSettings ──────────────────────────────────────────────────────────
function BackendSettings({onClose,onSaved}){
  ...
  return(<div style={{position:'fixed',inset:0,zIndex:300,background:'rgba(0,0,0,0.3)',display:'flex',alignItems:'center',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div style={{background:'#fff',border:`1px solid ${C.border}`,borderRadius:14,padding:28,width:500,boxShadow:'0 8px 40px rgba(0,0,0,0.12)'}}>
      ...
      <input value={url} onChange={e=>setUrl(e.target.value)} placeholder={envUrl||'https://your-backend.vercel.app'}
...
export default function App(){
  ...
  const[backendOk,setBackendOk]=useState(null) // null=unchecked, true=ok, false=fail

  // Check backend health on load and every 30s
  useEffect(()=>{
    const check=async()=>{
      try{
        const r=await fetch(getApiUrl()+'/health',{signal:AbortSignal.timeout(4000)})
        setBackendOk(r.ok)
      }catch(e){setBackendOk(false)}
    }
    check()
  ...
  return(<div style={{height:'100vh',background:C.bg,fontFamily:"'Inter','Space Grotesk',sans-serif",color:C.text,display:'flex',flexDirection:'column',overflow:'hidden'}}>
    ...
    {showBackend&&<BackendSettings onClose={()=>setShowBackend(false)} onSaved={async()=>{try{const r=await fetch(getApiUrl()+'/health',{signal:AbortSignal.timeout(4000)});setBackendOk(r.ok)}catch(e){setBackendOk(false)}}}/>}

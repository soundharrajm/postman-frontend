...
function HistoryPanel({history,onSelect,onClose}){
  return(<div style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,0.3)',display:'flex',alignItems:'flex-end',justifyContent:'flex-end'}} onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div style={{width:360,height:'100vh',background:'#fff',borderLeft:`1px solid ${C.border}`,display:'flex',flexDirection:'column',boxShadow:'-4px 0 20px rgba(0,0,0,0.08)'}}>
      ...
      <div style={{flex:1,overflowY:'auto'}}>
        {history.length===0&&<div style={{padding:20,fontSize:12,color:'#cbd5e1',textAlign:'center'}}>No history yet</div>}
        {history.map((h,i)=>{const mc=MC[h.method]||MC.GET;const sc=SC(h.status);return(
          <div key={i} onClick={()=>{onSelect(h);onClose()}} style={{padding:'10px 16px',borderBottom:`1px solid ${C.border}`,cursor:'pointer'}} onMouseEnter={e=>e.currentTarget.style.background='#f8f8fc'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
            ...
            <div style={{fontSize:11,color:'#94a3b8',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:C.mono}}>{h.url}</div>
          </div>
        )})}
      </div>
    </div>
...
export default function App(){
  ...
  const sendReq=async(url)=>{
    ...
    try{
      ...
      setHistory(h=>[{method:activeReq.method,url,status:data.status,time:new Date().toLocaleTimeString()},...h.slice(0,99)])
